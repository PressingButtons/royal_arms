export default function compile(gl, url) {
  ...
  function step01(json) {
    ...
    return Promise.all([Utils.fetch.text(json.vertex), Utils.fetch.text(json.fragment)]).then(step02)
  }

  if(typeof url === "string")
    return Utils.fetch.json(url).then(step01)
  else
    return Utils.fetch.resolve(step01(url));
}
